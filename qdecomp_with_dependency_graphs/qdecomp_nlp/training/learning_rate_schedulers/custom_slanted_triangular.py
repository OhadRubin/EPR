...
@LearningRateScheduler.register("custom_slanted_triangular")
class CustomSlantedTriangular(LearningRateScheduler):
    """
    ...
    def __init__(
            ...
            freeze_while_warmup: List[int] = None,
    ) -> None:
        ...
        self.freeze_while_warmup = freeze_while_warmup
        self.already_frozen: bool = False
    ...
    def get_values(self):
        ...
        # freeze params groups on warmup stage
        if self.freeze_while_warmup:
            should_freeze = step < cut
            if should_freeze != self.already_frozen:
                for i in self.freeze_while_warmup:
                    for param in self.optimizer.param_groups[i]["params"]:
                        param.requires_grad = not should_freeze
                self.already_frozen = should_freeze
