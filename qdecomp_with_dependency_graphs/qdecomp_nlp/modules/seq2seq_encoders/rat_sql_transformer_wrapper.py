"""
Based on https://github.com/microsoft/rat-sql/blob/master/ratsql/models/transformer.py
"""
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, dropout,
                 num_relation_kinds=None,
                 relation_k_embedder: TokenEmbedder = None, relation_v_embedder: TokenEmbedder = None
                 ):
        ...
        self.hidden_size = self.self_attn.d_k
        assert relation_k_embedder or num_relation_kinds, "pass a relation_embedder or the expected embeddings number"
        assert relation_v_embedder or num_relation_kinds, "pass a relation_embedder or the expected embeddings number"
        self.relation_k_emb = relation_k_embedder or Embedding(self.hidden_size, num_relation_kinds)
        self.relation_v_emb = relation_v_embedder or Embedding(self.hidden_size, num_relation_kinds)
        assert self.relation_k_emb.get_output_dim() == self.hidden_size and self.relation_v_emb.get_output_dim() == self.hidden_size, "mismatched sizes"
