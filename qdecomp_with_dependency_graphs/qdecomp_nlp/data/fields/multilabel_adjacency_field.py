"""
Based on allennlp/data/fields/adjacency_field.py
tag: v1.1.0
"""
...
class MultiLabelAdjacencyField(Field[torch.Tensor]):
    ...
    def as_tensor(self, padding_lengths: Dict[str, int]) -> torch.Tensor:
        ...
        tensor = torch.ones(desired_num_tokens, desired_num_tokens, self._num_labels) * self._padding_value
        labels = self._indexed_labels or [1 for _ in range(len(self.indices))]

        for index, label in zip(self.indices, labels):
            tensor[index][:] = 0
            tensor[index][label] = 1
        return tensor
    ...
    def empty_field(self) -> "MultiLabelAdjacencyField":
        ...
        adjacency_field = MultiLabelAdjacencyField(
