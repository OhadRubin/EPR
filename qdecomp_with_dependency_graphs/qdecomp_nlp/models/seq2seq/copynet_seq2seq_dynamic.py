"""
based on allennlp_models/generation/models/copynet_seq2seq.py
tag: v1.1.0
"""
import logging
...
@Model.register("copynet_seq2seq_dynamic")
class CopyNetSeq2SeqDynamic(Model):
    """
    ...
    def forward(self,  # type: ignore
                ...
                allowed_tokens: TextFieldTensors,
        ...
        if self._tie_weights:
            ...
            state["allowed_mask"] = util.get_text_field_mask(allowed_tokens)
            ...
            if target_tokens:
                ...
                if self._token_based_metric is not None:
                    ...
                    self._token_based_metric(  # type: ignore
                        predicted_tokens, [x["target_tokens"] for x in metadata], metadata
                    )
    ...
    def _get_generation_scores(self, state: Dict[str, torch.Tensor]) -> torch.Tensor:
        fill_value = -1e32
        if self._tie_weights:
            batch_size, embedding_dim, *_ = state["decoder_hidden"].size()
            ...
            generation_scores = self._adjust_target_outputs(output_projections, state["allowed_token_ids"],
                                                            state["allowed_mask"], fill_value=fill_value)
            dynamic_mask = (generation_scores != fill_value).float()
        else:
            generation_scores = self._output_generation_layer(state["decoder_hidden"])
            dynamic_mask = torch.zeros_like(generation_scores).scatter(dim=1, index=state["allowed_token_ids"], value=1.)
            dynamic_mask[:,0] = 0  # omit padding index
            fill_mask = (dynamic_mask == 0).long()
            generation_scores.scatter_(dim=1, index=fill_mask, value=fill_value)

        return generation_scores, dynamic_mask
    ...
    def _forward_loss(self,
        ...
        for timestep in range(num_decoding_steps):
            ...
            # shape: (batch_size, target_vocab_size)
            generation_scores, dynamic_mask = self._get_generation_scores(state)
            ...
            step_mask.append(torch.cat([generation_scores_mask, source_mask],dim=-1).unsqueeze(1))
    ...
    def take_search_step(
        ...
        #             dim=-1,
        #         )
        mask = torch.cat((dynamic_mask, source_mask), dim=-1)
        # Normalize generation and copy scores.
        # shape: (batch_size, target_vocab_size + source_sequence_length)
        log_probs = util.masked_log_softmax(all_scores, mask)
    ...
    default_predictor = "seq2seq_dynamic"