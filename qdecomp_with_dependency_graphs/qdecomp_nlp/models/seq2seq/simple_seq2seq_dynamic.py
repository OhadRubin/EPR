"""
Based on SimpleSeq2Seq. allennlp_models/generation/models/simple_seq2seq.py
tag: v1.1.0
"""
...
from qdecomp_nlp.models.util import masked_sequence_cross_entropy_with_logits


@Model.register("simple_seq2seq_dynamic")
class SimpleSeq2SeqDynamic(Model):
    """
    This `SimpleSeq2SeqDynamic` class is a `Model` which takes a sequence, encodes it, and then
    uses the encoded representations to decode another sequence, limited to some allowed tokens.
    ...
    def _prepare_output_projections(
        ...
        # shape: (group_size, num_classes)
        if self._target_decoder_layers > 1:
            last_decoder_hidden = decoder_hidden[-1]
        else:
            last_decoder_hidden = decoder_hidden
        # output_projections = self._output_projection_layer(last_decoder_hidden)
        batch_size, embedding_dim, *_ = last_decoder_hidden.size()
        num_allowed_tokens = state["allowed_embedded_input"].size(1)
        hidden_state = last_decoder_hidden.view(batch_size, 1, embedding_dim)
