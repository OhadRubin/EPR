...
class Decomposition(object):
    ...
    def to_break_standard_string(self, multiline=False):
        def format_step(step: str):
            step = step.replace("  ", " ").strip()
            if step.startswith('return'):
                return step
            return f'return {step}'
        steps = [format_step(x) for x in self.decomposition_list]
        if multiline:
            decomp = '\n'.join(f'{i+1}. {step}' for i, step in enumerate(steps))
        else:
            decomp = " ;".join(steps)
        return re.sub(r'@@(\d+)@@', '#\g<1>', decomp)
    ...
    @staticmethod
    def from_str(text, sep="@@SEP@@", ref_regex=r"@@(\d+)@@"):
        if ref_regex:
            text = re.sub(ref_regex, "@@\g<1>@@", text)
    ...
    def draw_decomposition(self):
        ...
        return draw_decomposition_graph(graph)
...
def draw_decomposition_graph(graph, title=None, pos=None):
    ...
    imgdata = io.StringIO()
    # plt.savefig('_debug/normal_form/a.png')
